...
#     plt.legend()
#     plt.show()
def show_cluster_graph(data, label, output_file=None, prefix="chart"):
    # Dimensionality reduction to 2D using t-SNE
    tsne = TSNE(n_components=2, random_state=42)
    data_2d = tsne.fit_transform(data)
    ...
    if output_file:
        # Add a timestamp prefix to the file name
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")  # Format: YYYYMMDD_HHMMSS
        output_file_with_prefix = f"{prefix}_{timestamp}_{output_file}"
        plt.savefig(output_file_with_prefix)
        print(f"Chart saved to {output_file_with_prefix}")
# ============================================================================================
